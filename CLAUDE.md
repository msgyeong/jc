...
6. **사용자 관점 개발 (강제 — 예외 없음)**: 아래 "사용자 관점 개발 규칙" 섹션 참고

## 사용자 관점 개발 규칙 (강제 — 예외 없음)

> **"기능이 동작한다 ≠ 완성이다"**
> 사용자가 실제로 쓸 수 있어야 완성이다. 이쁘고 편하게 쓸 수 있어야 완성이다.

### 1. 화면 구현 시 필수 체크
- 버튼이 다른 요소(하단 네비, 키보드, 모달)에 **가려지지 않는가**
- 입력 필드 크기와 스타일이 같은 화면 내에서 **일관적인가** (폭, 높이, border, padding)
- 텍스트가 잘리거나 넘치지 않는가 (**text-overflow 처리**)
- 정보가 **구분 가능한가** (예: SNS 종류, 카테고리 등 — 라벨/아이콘 필수)
- 사용자가 **무엇을 입력해야 하는지 알 수 있는가** (placeholder, 라벨 명확)
- 터치 영역이 충분한가 (**최소 44×44px**)
- 스크롤 최하단 콘텐츠가 하단 네비에 가려지지 않는가 (**padding-bottom 확보**)

### 2. 폼/입력 화면 필수 체크
- 모든 입력 필드가 **같은 너비(width: 100%)** 인가
- textarea가 일반 input과 **같은 디자인 언어**(border, radius, padding)를 쓰는가
- 선택형 입력(드롭다운, 라디오)에 **선택지가 명확한가**
- 저장/제출 버튼이 **항상 보이고 누를 수 있는가** (하단 고정 또는 충분한 여백)
- 필수 필드 표시가 있는가 (* 또는 "필수" 라벨)
- 유효성 검증 에러가 **해당 필드 바로 아래에** 표시되는가

### 3. 목록/카드 화면 필수 체크
- 카드 간 간격이 **일정한가**
- 같은 유형의 카드가 **같은 높이/레이아웃**인가
- 빈 목록일 때 **빈 상태 UI**가 나오는가 ("등록된 내용이 없습니다")
- 목록이 길 때 **마지막 항목이 하단 네비에 가려지지 않는가**

### 4. 완료 판단 기준
- "API 200 OK" = 완료가 **아니다**
- "화면이 렌더링됨" = 완료가 **아니다**
- **사용자가 실제로 정보를 읽고, 입력하고, 버튼을 누르고, 결과를 확인할 수 있어야 완료다**
- 이 체크리스트를 통과하지 못한 화면은 "완료"로 보고하지 마라
