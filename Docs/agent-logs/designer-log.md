...
| # | 파일 | 상태 | 설명 |
|---|------|------|------|
| — | `tokens.css` | **v2.2 최신** | CSS 디자인 토큰 (라이트+다크, 인물카드/직책/참석/Push/관리권한 토큰) |
| — | `components.md` | **v2.2 최신** | 컴포넌트 가이드 (§16~§19 + §20 앱 관리 권한 UI) |
...
### 작업 8: 인물카드 + 게시글 참석 + Push 설정 + 직책 뱃지 디자인
- **시간**: 2026-03-14
- **지시**: CEO — 기획서 4건 기반 디자인 시스템 업데이트
- **참조 문서**:
  - `Docs/features/member-dossier-plan.md` (인물카드 기획서 — 핵심)
  - `Docs/features/post-attendance-plan.md` (게시글 참석)
  - `Docs/features/member-position-plan.md` (직책 관리)
  - `Docs/features/push-notification-plan.md` (Push 알림 상세설정)
- **산출물**:
  - `Docs/design-system/tokens.css` — **v2.1 업데이트**
    - Dossier 헤더/아바타/탭/타임라인 토큰 추가
    - 정보 그리드 (라벨+값) 토큰 추가
    - 직책 뱃지 7종 색상 토큰 (회장=금, 수석부회장=은, 부회장/총무=청, 이사/감사=연청, 일반=회색)
    - 참석/불참 버튼 색상 토큰 (초록/빨강/회색)
    - Push 설정 라디오/체크박스 토큰
    - 다크 모드 대응 토큰 추가
  - `Docs/design-system/components.md` — **v2.1 업데이트** (3개 섹션 추가)
    - §16: 인물카드 (Dossier) — 상단 프로필 카드, 수평 스크롤 탭 8개, 정보 그리드, 타임라인, 교육/경력 카드, 가족 테이블, 관리자 메모, 반응형
    - §17: 직책 뱃지 (Position Badge) — 7종 색상 매핑, 사용 위치 정의
    - §18: 게시글 참석/불참 UI — 버튼(초록/빨강/회색), 현황 요약, 명단, 작성 폼 토글, 목록 카드 요약
    - §19: Push 알림 설정 UI — 라디오 4종(즉시/예약/D-day/없음), 날짜 입력, D-day 체크박스
    - 모든 섹션에 HTML 사용 예시 포함
- **디자인 결정 사항**:
  - 인물카드 프로필 사진: 120×120px (480px 이하에서 80×80px 축소)
  - 탭 8개: 수평 스크롤, scrollbar 숨김, sticky 고정
  - 직책 뱃지: font-size 11px, 테두리 포함 (차별화)
  - 회장 뱃지: 금색(#FEF3C7) — 유일한 따뜻한 색상 허용 (CEO 요구)
  - 참석/불참 버튼: 일정 상세(12번 문서)와 동일한 스타일 유지
  - Push D-day 옵션: 일정 미첨부 시 비활성(opacity 0.4)
- **상태**: 완료
...
### 작업 9: 앱 관리 권한 UI 디자인 가이드
- **시간**: 2026-03-15
- **지시**: CEO — 기획서 `mobile-admin-plan.md` 기반 디자인 시스템 업데이트
- **참조 문서**:
  - `Docs/features/mobile-admin-plan.md` (앱 내 관리자 권한 기획서)
- **산출물**:
  - `Docs/design-system/tokens.css` — **v2.2 업데이트**
    - 관리 뱃지 색상 (연한 블루 `#EFF6FF` / `#1E40AF` / `#93C5FD`)
    - 관리 허브 카드, 대기 건수 배지, 관리 액션 버튼 토큰
    - 긴급 발송 버튼 (error 계열), 권한 토글 행 토큰
    - 다크 모드 대응 추가
  - `Docs/design-system/components.md` — **v2.2 업데이트** (§20 추가)
    - §20-1: 관리자 뱃지 (badge-admin, 방패 아이콘)
    - §20-2: 관리자 웹 — 권한 관리 토글 UI (토글 행 + 사유 입력 + 변경 이력)
    - §20-3: 앱 — 프로필 관리 메뉴 배치
    - §20-4: 앱 — 관리 허브 화면 (카드 목록 + 대기 건수 배지)
    - §20-5: 앱 — 회원가입 승인 화면 (카드 + 승인/거부 버튼)
    - §20-6: 앱 — 게시글/일정 인라인 관리 버튼 (수정=블루, 삭제=빨강)
    - §20-7: 앱 — 긴급 푸시 발송 화면 (제목/내용/대상 라디오/발송 버튼/확인 다이얼로그)
    - §20-8: 디자인 원칙 요약 (6개 원칙)
    - §20-9: CSS 네이밍 컨벤션
    - 전체 HTML 사용 예시 포함
- **디자인 결정 사항**:
  - 관리 색상: 연한 블루(`#EFF6FF` + `#1E40AF`) — Primary(딥네이비)와 구분되는 밝은 블루
  - 관리 영역 표식: 🛡️ 방패 아이콘 일관 사용
  - 위험 액션(삭제/발송): error 계열 빨강 강조
  - 권한 없는 사용자: `display: none` 완전 숨김 (흐리게가 아님)
  - 승인/거부 버튼: approve=Primary, reject=Ghost+Red (시각적 비대칭으로 승인 유도)
  - 긴급 발송 버튼: 빨간색 배경 (위험성 강조) + 확인 다이얼로그 필수
- **상태**: 완료
