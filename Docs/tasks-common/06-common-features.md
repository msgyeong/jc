...
- [x] **웹** 디자인 시스템 (색상, Noto Sans KR, 버튼/카드/입력 스타일) — `web/styles/main.css`
- [x] **웹** 로딩/빈 상태/에러 표시 (content-loading, empty-state, error-state) — 각 화면 JS
- [x] **웹** 날짜 포맷 (formatRelativeTime, formatDate, isNewContent) — `web/js/utils.js`, `web/js/home.js`
- [x] **웹** N배지 (3일 이내 isNewContent) — 게시글/공지/일정 카드
- [x] **웹** 공통 유틸 (escapeHtml, setButtonLoading, debounce) — `web/js/utils.js`
- [ ] **웹** 이미지 업로드 유틸 (게시글/프로필 등 다중 업로드)
...
- [x] **(웹)** 공통 에러 처리가 모든 화면에 적용됨 (error-state, 인라인 에러)
- [x] **(웹)** 로딩 상태가 적절하게 표시됨 (content-loading, 버튼 스피너)
- [x] **(웹)** 빈 상태가 모든 화면에 적용됨 (empty-state)
- [ ] 이미지 업로드가 여러 화면에서 정상 작동함
- [x] **(웹)** 날짜/시간이 일관되게 포맷팅됨 (formatRelativeTime, formatDate)
- [x] **(웹)** 앱 아이콘 및 스플래시 스크린이 설정됨 (스플래시 화면, 로고)
- [ ] 전체 기능이 통합되어 정상 작동함
